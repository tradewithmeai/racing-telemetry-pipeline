...
def resample_with_interpolation(
    ...
    """Resample with interpolation instead of forward-fill.

    This is an alternative to forward-fill that uses interpolation
    for smoother results. Better for continuous signals like speed, position.
    ...
    """
    logger.info("="*60)
    logger.info(f"RESAMPLE (Interpolation): {chassis_id}")
    logger.info("="*60)
    ...
    for col in df_resampled.columns:
        if col in ["chassis_id", "car_no", "lap_repaired", "segment_id"]:
            # Use forward-fill for discrete values
            df_resampled[col] = df_resampled[col].ffill()
        else:
            # Use interpolation for continuous signals
